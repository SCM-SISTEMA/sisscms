<?php
	class UsuarioController extends Zend_Controller_Action {
		...
		public function indexAction()
	    {
	    	...
	    	$usuarios = Model_Usuario::getInstance()->getConsultaAll( $arrParam );
		 ...
		 *
		 */
		public function buscausuariocpfAction() {
			...
			// Unidade(s)
			$arrUnidade = Model_UsuarioUorg::getInstance()->getUnidadesUsuario($this->limpaCaracterCpf());
			
			// Informações do usuário
			$Usuario = Model_Usuario::getInstance()->getUsuarioPerfil($this->limpaCaracterCpf());
			$Usuario['arrUnidade'] = $arrUnidade;
			
		    // Estagio Probatorio
		    $Servidor = Model_CadastroServidor::getInstance()->getEstadoProbatorio($this->limpaCaracterCpf());

		    $Usuario['ST_PROBATORIO'] = ($Servidor ? $Servidor['ST_PROBATORIO'] : '');
		   	
			$this->_response->appendBody(Zend_Json::encode($Usuario));			
		}
