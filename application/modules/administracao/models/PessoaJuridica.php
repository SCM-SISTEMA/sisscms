...
class Administracao_Model_PessoaJuridica extends Administracao_Model_Db
{
	...
	public function getConsultaEmail($arrParam) {
		$sql = $this->getAdapter()->select()->from($this->_name,array('ds_email'),$this->_schema)
		->order(array('no_razao_social'));

		$arrBind = array();

		if(key_exists('co_pessoa_juridica',$arrParam) && $arrParam['co_pessoa_juridica']){
			$sql->where($this->_name.'.co_pessoa_juridica in ('.$arrParam['co_pessoa_juridica'].')');
		}
	...
	public function getConsulta($arrParam) {
		...
		->joinLeft('tb_estados', $this->_name.'.sg_uf = tb_estados.co_estado', array('sg_estado'))
		->order(array('no_razao_social'));
		...
		if(key_exists('nu_cnpj',$arrParam) && $arrParam['nu_cnpj']){
			$sql->where("nu_cnpj = ?");
			$arrBind[] = $arrParam['nu_cnpj'];
		}
