...
class PHPJasperXML {
    ...
    public function PHPJasperXML($lang="en",$pdflib="TCPDF") {
       ...
       $this->setErrorReport(0);
        ...
        $this->pdflib=$pdflib;
        if($this->fontdir=="")
        $this->fontdir=APPLICATION_PATH . '/../library/phpjasperxml/class/tcpdf/fonts';
    }
    ...
    $this->db_host=Zend_Registry::get('config')->resources->db->params->host;
            $this->db_user=Zend_Registry::get('config')->resources->db->params->username;
       $this->db_pass=Zend_Registry::get('config')->resources->db->params->password;
    $this->db_or_dsn_name=Zend_Registry::get('config')->resources->db->params->dbname;
    $this->cndriver="mysql";
        if($this->cndriver=="mysql") {
            ...
            if(!$this->con) {
                ...
                if($this->myconn) {
                    $seldb = @mysql_select_db($this->db_or_dsn_name,$this->myconn);
                    if($seldb) {
                        $this->con = true;
                        return true;
                    }
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray() {
        ...
        if(!$this->connect())	//connect database
        {
